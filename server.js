...
class FinanceCalculator {
    ...
    static calculatePortfolioGrowth(initialAmount, monthlyContribution, annualRate, years) {
        ...
        const futureValueContributions = this.calculateFutureValueAnnuity(
            monthlyContribution,
            monthlyRate,
            totalMonths
        );
    ...
    static generateYearlyGrowthData(initialAmount, monthlyContribution, annualRate, years) {
        ...
        for (let year = 1; year <= years; year++) {
            ...
            const fvContributions = this.calculateFutureValueAnnuity(
                monthlyContribution,
                monthlyRate,
                months
            );
...
class CalculationService {
    static calculateInvestmentScenarios(params) {
        ...
        const scenarios = CONFIG.PORTFOLIOS.map(portfolio => {
            const totalValue = FinanceCalculator.calculatePortfolioGrowth(
                initialAmount,
                monthlyContribution,
                portfolio.rate,
                years
            ...
            return {
                name: portfolio.name,
                fv: FinanceCalculator.roundToTwoDecimals(totalValue),
        ...
        const datasets = CONFIG.PORTFOLIOS.map(portfolio => ({
            ...
            data: FinanceCalculator.generateYearlyGrowthData(
                initialAmount,
                monthlyContribution,
                portfolio.rate,
                years
        ...
        const totalContributionsData = Array.from({ length: years + 1 }, (_, i) =>
            initialAmount + (monthlyContribution * i * 12)
        );
    ...
    static calculateWithdrawalScenarios(params) {
        ...
        const scenarios = CONFIG.WITHDRAWAL_AMOUNTS.map(withdrawal => {
            ...
            CONFIG.PORTFOLIOS.forEach((portfolio, index) => {
                scenario[portfolio.key] = FinanceCalculator.simulateWithdrawal(
...
app.post('/calculate',
    validateRequest(InputValidator.validateCalculateRequest),
    (req, res) => {
        try {
            ...
            res.status(500).json({
                ...
                message: 'Unable to process investment calculation'
            });
        }
    }
);

app.post('/withdrawal-scenarios',
    validateRequest(InputValidator.validateWithdrawalRequest),
    (req, res) => {
        try {
            ...
            res.status(500).json({
                error: 'Withdrawal calculation failed',
                message: 'Unable to process withdrawal scenarios'
            });
        }
    }
);
...
const server = app.listen(CONFIG.PORT, () => {
    ...
    console.log(`   POST /calculate - Cálculo de investimentos`);
    console.log(`   POST /withdrawal-scenarios - Cenários de saque`);
